package com.iftm.api.agendadecompromisso.exceptions.handler;

import com.iftm.api.agendadecompromisso.exceptions.ExceptionResponse;
...
@RestController
@ControllerAdvice // Componente que pode tratar exceções lançadas em diferentes

// controladores dentro da aplicação.
public class CustomizedResponseEntityExceptionHandler extends
    ...
    handlwBadRequestException(Exception ex, WebRequest request) {
        var exceptionResponse = new ExceptionResponse(new Date(),
                ex.getMessage(),
                request.getDescription(false));
